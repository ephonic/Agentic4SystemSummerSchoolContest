...
5. 完成 XDMA 驱动/运行时、指令编译工具和 PyTorch 自定义后端或算子接入；
...
- 必须提供统一 kernel launch、内存分配、数据传输、同步和错误返回接口；
- 必须通过公开测试、随机测试、隐藏测试和赛后生成的未知 kernel 测试；
- 必须提交可复现RTL代码以及工程、bitstream/xclbin、软件栈源码、资源/时序报告、评分日志和设计说明。
...
参赛处理器必须兼容附件定义的 AEC 128-bit 定长格式：
...
完整模型执行必须支持下列扩展。编码由 AEC-G v1.0、机器可读 ISA 描述和 golden simulator 固定，禁止由参赛队伍各自解释：
...
基础编译器沿用参考题目：输入 NVIDIA PTX 9.3 的受限子集，输出 `.aecbin` 和 JSON 编译报告。
...
提供可工作的 XDMA shell 和基础 Linux 驱动，使参赛重点放在 GPGPU 而非重新实现 PCIe endpoint。参赛队伍必须实现或扩展用户态 runtime；驱动创新可加分。
...
| 项目 | 分值 | 机器可执行口径 |
|---|---:|---|
| 编译器正确性和优化 | 5 | `5 × weighted_pass_rate(compiler_manifest)` |
| PyTorch 接入与 fallback 日志 | 3 | `3 × weighted_pass_rate(pytorch_manifest)` |
| 驱动/runtime 的异步、错误处理和可观测性 | 2 | `2 × weighted_pass_rate(runtime_manifest)` |
...
driver/              # 内核态改动（如有）
runtime/              # C/C++ runtime 与 API
compiler/             # PTX/AEC 编译器、汇编器、反汇编器
pytorch/              # Python package、算子和模型脚本
tests/                # 单元、随机、板上测试
...
- 每项成绩对应的原始日志和可复现实验命令；
- `design.json`，记录 CU、`logical_warp_width`、`physical_simd_lanes`、`issue_beats_per_warp`、cache、GEMM、频率和数值模式。
